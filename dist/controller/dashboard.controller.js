...
export const getPmStats = async (req, res) => {
    try {
        ...
        const projectSummary = pmProjects.map((p) => {
            ...
            const overdue = p.tasks.filter((t) => t.isOverdue || (t.status !== TaskStatus.DONE && new Date(t.dueDate) < new Date())).length;
            return {
                id: p.id,
                name: p.name,
...
export const getDeveloperStats = async (req, res) => {
    try {
        ...
        const overdue = tasks.filter((t) => t.isOverdue || (t.status !== TaskStatus.DONE && new Date(t.dueDate) < new Date())).length;
        res.status(200).json({
            success: true,
            stats: {
                total,
